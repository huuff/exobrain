...
* `ENTRYPOINT`: Like the `CMD` command, specifies a command to be run on execution. Contrary to the `CMD` command, it can't be overriden and will always be run, any extra arguments the user passes to the `docker run` command will be passed as arguments to the `ENTRYPOINT` command. It's a bad practice to run a default command as an `ENTRYPOINT` unless you want to pass through any arguments, otherwise you should use `CMD` so it can be overriden by the user to debug or interact with the container.
